...
Syntax:
    ...
    docker run -v [volume-name]:/volume --rm aouziel/volume-backup backup - > [archive-name]

For example:

    docker run -v some_volume:/volume --rm aouziel/volume-backup backup - > some_archive.tar.bz2
...
Syntax:
    ...
    docker run -v [volume-name]:/volume -v [output-dir]:/backup --rm aouziel/volume-backup backup [archive-name]

For example:

    docker run -v some_volume:/volume -v /tmp:/backup --rm aouziel/volume-backup backup some_archive
...
Syntax:
    ...
    cat [archive-name] | docker run -i -v [volume-name]:/volume --rm aouziel/volume-backup restore -

For example:

    cat some_archive.tar.bz2 | docker run -i -v some_volume:/volume --rm aouziel/volume-backup restore -
...
Syntax:
    ...
    docker run -v [volume-name]:/volume -v [output-dir]:/backup --rm aouziel/volume-backup restore [archive-name]

For example:

    docker run -v some_volume:/volume -v /tmp:/backup --rm aouziel/volume-backup restore some_archive
...
Syntax:
    ...
    docker run -v [volume-name]:/volume --rm aouziel/volume-backup backup - |\
         ssh [receiver] docker run -i -v [volume-name]:/volume --rm aouziel/volume-backup restore -
...
For example:
    ...
    docker run -v some_volume:/volume --rm aouziel/volume-backup backup -c none - |\
         ssh user@new.machine docker run -i -v some_volume:/volume --rm aouziel/volume-backup restore -c none -
    
## Miscellaneous

1. Upgrade / update volume-backup
    ```
    docker pull aouziel/volume-backup
    ```
...
1. Exclude some files from the backup and send the archive to stdout
    ```
    docker run -v [volume-name]:/volume --rm aouziel/volume-backup backup -e [excluded-glob] - > [archive-name]
    ```

1. Use different compression algorithm for better performance
    ```
    docker run -v [volume-name]:/volume --rm aouziel/volume-backup backup -c gz - > [archive-name]
    ```
1. Show simple progress indicator using verbose `-v` flag (works both for backup and restore)
    ```
    docker run -v [volume-name]:/volume --rm aouziel/volume-backup backup -v > [archive-name]
    ```
